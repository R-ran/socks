...
export default function CheckoutPageContent() {
  ...
  const handlePayPalPayment = async (data: unknown, actions: unknown) => {
    try {
      // Type assertion for actions
      const paypalActions = actions as PayPalActions;
      ...
      const order = await paypalActions.order.create({
        purchase_units: [
          {
            amount: {
              currency_code: 'USD',
  ...
  const handlePayPalApprove = async (data: unknown, actions: unknown): Promise<void> => {
    try {
      const paypalActions = actions as PayPalActions;
